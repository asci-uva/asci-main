...
export function tokenStatusMessage(status) {
    if (!status.hasPrimaryInstructor)
        return "This course has no primary instructor, so Canvas LMS is unavailable.";
    if (!status.hasToken)
...
function CanvasLmsSync(props) {
    ...
    const confirmRemoveCanvasLmsCourse = () => {
        ...
        const payload = {
            asciCourseId: props.course_id,
            command: "unlinkcCanvasLmsCourse"
        };
        ...
        postCommand(props.url, payload)
            .then((data) => {
                if (data.success === "true") {
                    ...
                    setSelectedYear("");
                    getCanvasLmsCourses();
                    console.log("Successfully unlinked with Canvas LMS course");
                    toast.success("Successfully unlinked with Canvas LMS course");
                } else {
                    toast.error(data.error);
                }
            })
    ...
    return (
        <div className="card">
            <h4 className="card-header">Course Linking</h4>
                ...
                {!props.canvasLmsCourseLoaded ? (
                    ...
                    <>
                        <div className="mb-3">
                            ...
                            {isPrimaryInstructor && (
                                <button type="button" className="btn btn-primary" onClick={removeCanvasLmsCourse}>Unlink from course</button>
                            )}
                        </div>
