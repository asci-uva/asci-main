...
function Home(props) {
  ...
  //This function checks the TA shifts
  const getShift = (url0) => {
    let user = localStorage.getItem("asci-user");
    let courseId = localStorage.getItem("asci-course");
    let request = {};
    ...
    fetch(url0, {
      ...
      .then((data) => {
        ...
        let success = data.success;
        if (success === "true") {
          console.log("HOME: Received shift, routing to correct page");

          if (data.shift === null) {
            navigate(docRoot + "/TACheckIn");
          } else {
  ...
  function routeToCorrectPage(data, url0) {
