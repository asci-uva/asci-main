...
function StudentWaitingRoom(props) {
  ...
  const [numTAs, setNumTAs] = useState("Loading...");
    ...
    fetch(url0, {
      ...
      .then((data) => {
        ...
        if (data.success === "true") {
          ...
          if (polling == true) {
            timeoutId = setTimeout(pollNumTAs, 30000);
          }
        } else {
          console.log("Getting number of TAs failed");
      ...
      .then((data) => {
        ...
        if (data.success !== "true") {
          ...
          let waitTime = parseInt(
            (Date.now() - new Date(data.session.entry_time).getTime()) /
              1000 /
              60,
            10
          );
  ...
  return (
    <div className="question">
      <div>
        <h4>
          You are currently in the queue for {courseName}. A TA Will be with you
          shortly.{" "}
        </h4>
        <h4>There are currently {numTAs} TAs working. </h4>
        <h6>
          You have been waiting for <b>{minsWaiting} minutes</b>
        </h6>
      </div>
      <div>
